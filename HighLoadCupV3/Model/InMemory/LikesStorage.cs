...
{
    public class LikesStorage : ILikesStorage
    {
        ...
        public LikesStorage(int accountsCount)
        {
        ...
        public async Task<List<int>> GetFromAsync(int id)
        {
            ...
            if (_fromFilePositions[id, 1] != 0)
            {
                ...
                using (var reader = new StreamReader(File.OpenRead(FromFileName)))
                {
                    reader.BaseStream.Seek(_fromFilePositions[id, 0], SeekOrigin.Begin);
        ...
        public async Task<List<LikeDto>> GetToAsync(int id)
        {
            ...
            {
                using (var reader = new StreamReader(File.OpenRead(ToFileName)))
                {
                    reader.BaseStream.Seek(_toFilePositions[id, 0], SeekOrigin.Begin);
